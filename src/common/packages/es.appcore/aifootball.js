...
/**
 * @name AppCore.aifootball
 * @description ai足球
 * @namespace
 */
Apps.reg('aifootball', {
	is: function is(ua) {
		return /(^|\s)client\/aizuqiu/.test(ua);
	},
	ready: function ready(loginFn) {
		// 监听登录成功事件
		// @argument param {jsonstring}
		document.addEventListener('AIJsLogin', function (param) {
			var _param = null;
			// 如果是字符串格式
			if (typeof param === 'string') {
				_param = JSON.parse(param);
			}
			loginFn && loginFn(_param || param);
		}, false);
		// 暂时用不到jsbridge， 先注掉
		// if (window.AIJsApi) {
		// 	return jsBridgeFn(window.AIJsApi);
		// }
		// document.addEventListener('AIJsBridgeReady', () => {
		// 	jsBridgeFn && jsBridgeFn(window.AIJsApi);
		// }, false);
	}
});
...
/**
 * AI足球App登录模块
 * created by zhangruiqi 2018-7-4 11:27:35
 */
Apps.reg('aifootball', {
	login: function login(okUrl, cancelUrl, success, fail) {
		// 客户端dispatch AIJsLogin事件传回的param参数，已经在./is ready方法中json格式化处理
		AppCore$2.aifootball.ready(function (param) {
			if (!param || typeof param !== 'object') { return fail(param); }
			if (param.detail && param.detail.accountId && param.detail.sessionId) {
				success(param.detail);
			}
		});
		// 强制登录，清空客户端登录状态
		AppCore$2.sendCmd('aizuqiu://relogin');
		// AppCore.sendCmd('aizuqiu://login');
	}
});
