...
Apps.reg('tjwap', {
	is: function is (ua, os) {
		return /(?:^((guessing)|(g)|(m\d*?)|(wap\d*?)))\.tttaojin\.com/.test(location.host.toLowerCase());
	}
});
...
Apps.reg('tjwap', {
	login: function login(okUrl, cancelUrl, success, fail) {
		var host = window.location.protocol + "//" + 'g.rrzcp8.com';
		var redirectUrl = window.location.href;
		//2015-11-16 增加from参数的传递
		var from = (function() {
			if (/(?:\?|&)from=([^&$#]+)/i.test(location.search)) {
				return RegExp.$1;
			}
		})();
		fail('jump');
		//跳转页面
		window.location.replace([host + "/activity/guessingPage.html?okUrl=", encodeURIComponent(okUrl),
