...
/**
 * @name AppCore.kiwi
 * @description 现金贷控制方法集合
 * @namespace
 */
Apps.reg('kiwi', {
  is: function is(ua) {
    return /yuyin\sclient\/kiwifruit/.test(ua);
  },
  ready: function ready(fn) {
    if (window.KiwiJsApi) {
      return fn(window.KiwiJsApi);
    }
    document.addEventListener('KiwiJsBridgeReady', function () {
      fn(window.KiwiJsApi);
    }, false);
  }
});
...
Apps.reg('kiwi', {
  login: function login(okUrl, cancelUrl, success, fail) {
    AppCore$2.kiwi.ready(function (jsapi) {
      // 第一个参数给未来配置功能预留
...
Apps.reg(true, 'kiwi', {
	 ...
	 */
	closePage: function() {
		AppCore$2.kiwi.ready(function(api) {
			if (api && api.activity && api.activity.close) {
				 api.activity.close();
			}
		});
	}
