/*!
 * dialog.js v1.0.4
 * (c) 2014-2018 jianxcao
 * Released under the MIT License.
 */
(function (global, factory) {
	typeof exports === 'object' && typeof module !== 'undefined' ? module.exports = factory(require('vue')) :
	typeof define === 'function' && define.amd ? define(['vue'], factory) :
	(global.dialog = factory(global.Vue));
...
var renderFooter = function (createElement) {
	...
	var l = button.length;
	var nodes = button.map(function (cur, index$$1) {
		...
		return createElement('a', {
			...
			attrs: {
				'data-action': 'btn',
				'data-ret': globalConf.getBtnRetId(index$$1, l),
...
var fix = function () {
	...
	for (var i = 0; i < l; i++) {
		var cur = inputs[i];
		//  防止相同内容插入，所以做个解除操作
		cur.removeEventListener('focus', this$1.__evt_focus);
		cur.addEventListener('focus', this$1.__evt_focus);

		// 失去焦点
		cur.removeEventListener('blur', this$1.__evt_blur);
		cur.addEventListener('blur', this$1.__evt_blur);
	}
};
...
var rePosition = function (ele) {
	var scrollTop = Math.max(document.body.scrollTop, document.documentElement.scrollTop);
	var winHeight = window.innerHeight;
	// bottom表示元素距离页面上部的距离
	var pos = ele.getBoundingClientRect();
	var isFixed = this.dialogPos === 'fixed';
	var h = isFixed ? pos.bottom : pos.bottom + scrollTop;
	// 键盘打开高度却没有变化, 说明计算出错
...
(function () {
	...
	var reset = function () {
		...
		var isActiveElement = ele && /input|textarea|select/i.test(ele.tagName);
		if (!isActiveElement) {
			return;
		}
	...
	var resize = function () {
		...
		timer = window.setTimeout(function () {
			var result = dialogManager.getAllInstance();
