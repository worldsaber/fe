<template>
    <div class="m-keyboard" v-show="showKeyboard" tabindex="0" @focus="show" @blur="hide" @keydown="handleKeydown">
		  <div class="m-keyboard-row">
			<span class="m-keyboard-key" v-for="n in 5" :data-key="n" :key="n" @click.stop="change">{{n}}</span>
			<span class="m-keyboard-key" data-key="clear" @click.stop="change">
				<!-- svg关闭按钮直接内嵌 -->
		  ...
		  <div class="m-keyboard-row">
			<span class="m-keyboard-key" v-for="n in [6, 7, 8, 9, 0]" :data-key="n" :key="n" @click.stop="change">{{n}}</span>
