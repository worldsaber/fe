	...
	async mounted () {
		try {
			...
			if (ret.bizCode === 10000) {
				...
				insertIframe.id = 'luckyPoker';
				document.querySelector('#lucky-poker-app').appendChild(insertIframe);
				insertIframe.src = ret.data.luckyPokerUrl;
				this.changeLoading(false);
				// const origin = ret.data.luckyPokerUrl.split('?')[0].slice(0, -1);
				const origin = (new URL(ret.data.luckyPokerUrl)).origin;
