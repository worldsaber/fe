...
export default {
			...
			.then(ret => {
				...
				case 10000: {
					...
					if (status === 20) {
						...
						commit(mutationTypes.UPDATEPAGEMODULE, 'confirm');
					} else {
				...
				case 62100:
					commit(mutationTypes.UPDATEERRORINFO, {
						type: 'dialog',
						msg: ret.message || `Maximum Daily Transaction Value is ${showCurrency}${formatNumber(despositMax, 2)}. The maximum you can send in a day is ${showCurrency}${formatNumber(despositMax, 2)}.`
					});
					break;
				// 验证过程中信息没对上
				case 11000:
	...
	queryInfo({
		...
		if (!state.tradeId) {
			commit(mutationTypes.UPDATEPAGEMODULE, 'index');
			dialog();
			return;
		}
