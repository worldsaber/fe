		...
		<div v-if="Data">
			...
			<table>
				...
				<tr>
					...
					<td class="r-side">{{currency}} {{totalGiftAmount}}</td>
				</tr>
			</table>
			<div class="referral">
				<div class="flex" v-for="(item,i) in invData" :key="i">
					...
					<div class="units">
						...
						<div class="r" :class="[item.giftAmount != 0 ? 'positive' : 'negative']">{{currency}} {{item.giftAmount}}</div>
					</div>
					<div class="units">
						<div class="bars">
							<span>0</span>
	...
	methods: {
		...
		formatInvData (data) {
			...
			for (let i = 0; i < data.length; i++) {
				...
				if (data[i].curStake < 30 && data[i].curStake > 0) {
					data[i].curStake = 30;
				} else if (data[i].curStake > this.referrAmount) {
					data[i].curStake = this.referrAmount;
				}
				if (deviceWidth < 375) {
					data[i].activeBarLen = (195 * data[i].curStake / this.referrAmount) + 'px';
				} else if (deviceWidth >= 375 && deviceWidth < 415) {
					data[i].activeBarLen = (228 * data[i].curStake / this.referrAmount) + 'px';
				} else if (deviceWidth >= 415) {
					data[i].activeBarLen = (260 * data[i].curStake / this.referrAmount) + 'px';
				}
