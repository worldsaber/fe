...
```javascript
import Vue from 'vue'
import AfStep from 'element-steps'
...
```javascript
import Vue from 'vue'
import { AfSteps, AfStep } from 'element-steps'

Vue.component('m-steps', AfSteps)
Vue.component('m-step', AfStep)
```
...
| icon | 图标 | Afement Icon 提供的图标，如果要使用自定义图标可以通过 slot 方式写入 | string | — |
