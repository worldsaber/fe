...
/*
	betEvent default config
	对应的原始数据的key, 没有对应的key使用原始key
	{
		"topic": "2^15^sr:tournament:132^sr:match:12233936^betStatus",
		"betStatus": -1,
		"eventStatus": 1
	}
 */
const betEventDefaultKey = {
	topic: 'topic',
	betStatus: 'betStatus',
	eventStatus: 'status'
};
...
// outcome  推送消息需要更新的字段
const betEventUpdateDefaultKey = {
	topic: 1,
	betStatus: 1,
	status: 1
};
...
// push消息key
const keySet = ['odds', 'market', 'event', 'betEvent'];
...
function clearPushData(pushData, config, type) {
	...
	} else if (type === 'event' || type === 'betEvent') {
...
function getConfig(config, type) {
	...
	case 'event':
		...
		break;
	case 'betEvent':
		mappingKey = betEventDefaultKey;
		updateKey = betEventUpdateDefaultKey;
		break;
	default:
	}
