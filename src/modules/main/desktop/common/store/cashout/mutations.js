import { formatNumber, objType, isEmptyObject } from 'utils';
...
import { getCashoutTips, clearBetData, calPotWin } from './commonFun';
...
export default {
	...
	[mutationTypes.UPDATECASHOUTINFO](state, data = {}) {
		...
		// 优先显示code对应错误信息
		if (isEmptyObject(state.errorInfo)) {
			// 根据cashout的值，重置errorInfo
			const errorInfo = getCashoutTips(data, state.minCashoutAmount, state.maxCashOutAmount);
			if (errorInfo) {
	...
	// refresh errorinfo
	[mutationTypes.REFRESHERRORINFO](state) {
		const cashoutInfo = state.cashoutInfo || {},
			isSupportPartial = cashoutInfo.isSupportPartial || false;

		if (isSupportPartial && !cashoutInfo.cashoutFaild) {
			// 根据cashout的值，重置errorInfo
			const errorInfo = getCashoutTips(cashoutInfo, state.minCashoutAmount, state.maxCashOutAmount);
			if (errorInfo) {
				state.errorInfo = { ...errorInfo };
			}
		}
