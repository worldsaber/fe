export function addWrapper (htmlData, params) {
	const bodyStyle = 'font-family:' + params.font + '; font-size: ' + params.font_size + '; width:100%;';
...
export function collectStyles (element, params) {
  ...
  // String variable to hold styling for each element
    ...
    // Styles including
		const targetStyles = ['border', 'box', 'break', 'text-decoration'];

    // Exact match
		const targetStyle = ['clear', 'display', 'width', 'min-width', 'height', 'min-height', 'max-height', 'font-size', 'font-weight', 'float', 'line-height'];
...
export function loopNodesCollectStyles (elements, params) {
    ...
    // Form Printing - check if is element Input
      ...
      // Save style to variable
			const textStyle = collectStyles(currentElement, params);

      // Remove INPUT element and insert a text node
			const parent = currentElement.parentNode;

      // Get text value
			const textNode = tag === 'SELECT' ?
