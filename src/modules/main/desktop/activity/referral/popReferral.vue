		...
		<div v-if="Data">
			...
			<div class="total-info">
				...
				<div class="total">
					...
					<p>{{showCurrency}}{{totalGiftAmount}}</p>
				</div>
			</div>
			<div class="table">
			...
			<div class="table" v-for="item in invData">
				...
				<div class="arrow" :class="[item.referredCurStake===1000||item.referredCurStake>1000?'':'hide']"><span>……</span>></div>
				<div class="units" style="padding-left:40px">
					<div>{{showCurrency}}{{item.giftAmount}}</div>
				</div>
			</div>
			<Pagination
	...
	methods: {
		...
		formatInvData (data) {
			for (let i = 0; i < data.length; i++) {
				...
				if (data[i].curStake < 250 && data[i].curStake > 0) {
					data[i].curStake = 250;
				} else if (data[i].curStake > 1000) {
					data[i].curStake = 1000;
				}
				data[i].activeBarLen = (284 * data[i].curStake / 1000) + 'px';
