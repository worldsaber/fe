...
export default {
	...
	methods: {
		initParams() {
			...
			this.distance = 0;
			this.cycle = this.config.cycle || 40000;
			this.direction = this.config.direction || 'horizon';
		},
		reset() {
		...
		bindEvent() {
			...
			containerNode.addEventListener('touchstart', this._startHandle, true);
			containerNode.addEventListener('touchmove', this._moveHandle, true);
			containerNode.addEventListener('touchend', this._endHandle, true);
			containerNode.addEventListener('touchcancel', this._endHandle, true);
			...
			function startHandle (e) {
				// e.preventDefault();
				this.stop();
				startPos = this.direction === 'horizon' ? e.targetTouches[0].pageX : e.targetTouches[0].pageY;
				startPoint = {
			...
			function moveHandle (e) {
				const deltaX = Math.abs(e.targetTouches[0].pageX - startPoint.x);
				const deltaY = Math.abs(e.targetTouches[0].pageY - startPoint.y);

				if ((deltaX < deltaY && this.direction === 'horizon') || (deltaX > deltaY && this.direction === 'vertical')) {
					return;
				}
				e.preventDefault();

				const currentPos = this.direction === 'horizon' ? e.targetTouches[0].pageX : e.targetTouches[0].pageY;

				moveLen = currentPos - startPos;
				startPos = currentPos;

				const temp = this.distance - moveLen;

				if (temp < 0) {
					this.distance = 0;
				} else if (temp > this.length) {
		...
		handleResize() {
			...
			const parentOffset = this.calContainerOffset();
	...
	beforeDestroy() {
		const containerNode = this.$refs.container;
		containerNode.removeEventListener('touchstart', this._startHandle);
		containerNode.removeEventListener('touchmove', this._moveHandle);
		containerNode.removeEventListener('touchend', this._endHandle);
		containerNode.removeEventListener('touchcancel', this._endHandle);
