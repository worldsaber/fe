...
function convertEventsToMap (data = [], result = {}) {
	...
	// 存储 data 中 eventId 的原始顺序，
	result.originOrder = result.originOrder || [];
	const { order, map, originOrder } = result;
	data.forEach((current, index) => {
		const sport = current.sport;
		delete current.sport;
		originOrder.push(current.eventId);
		// 拍平sport字段
...
function updateEvent (map, tournamentOrder, eventInfo, topicInfo, productId, isInsert = true, isDel = true) {
	...
	if (map) {
		...
		if (event) {
					...
					for (const key in order) { // eslint-disable-line
						...
						if (index > -1) {
							...
							if (isEmptyObject(map[tournamentId].events)) {
								...
								// 修改 tournament eventSize 数量
								let size = map[tournamentId].eventSize;
								size = size - 1 < 0 ? 0 : size - 1;
								Vue.set(map[tournamentId], 'eventSize', size);
							}
							break;
						}
					}
			...
			// 修改 tournament eventSize 数量
			const size = map[tournamentId].eventSize || 0;
			Vue.set(map[tournamentId], 'eventSize', size + 1);
		}
	}
}
