		...
		<div class="m-sel-item">
			...
			>
				<div
					...
					:key="item.id"
				>
					<span>{{item.home}} <em class="m-vs">vs</em> {{item.away}}</span>
				</div>
			</template>
			<div
				class="m-text-mini"
				v-if="betInfo.showMores"
			>
				<span>{{betInfo.showMores}}</span>
			</div>
		</div>
...
export default {
	...
	computed: {
		list() {
			return this.betInfo.selectionDescs || [];
		},
		selections() {
